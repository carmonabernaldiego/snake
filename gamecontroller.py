...
class GameController:
    ...
    def check_game_over_for_player(self, player):
        if (len(self.get_possible_moves(player)) > 0):
            return False
        print(f"Se acabó el juego para {player.name}!")
        self._players.remove(player)
        return True

    def check_game_over(self):
        if (len(self._players) == 1):
            print(f"Jugador {self._players[0].name} gana!")
...
class MapCell:
    ...
    def __repr__(self):
        return f"Celda de {self.player} con {self.turnsLeft} gira a la izquierda"
...
class Player:
    ...
    def __repr__(self):
        return f"Jugador:{self.name}. Movido a {self.lastTurnMoved} en {self.position}"
